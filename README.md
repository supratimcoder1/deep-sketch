...
- Features a robust built-in preprocessing suite to format messy selfie inputs into pristine ID-style portraits so they behave predictably during inference
...
- An optional end-to-end script that chains both generators: `photo -> sketch -> stylized color`, with optional preprocessing to convert normal selfies into optimized CUFS-style photos before starting.
...
├── scripts/
│   ├── prepare_stylized_data.py  # Generates Phase 2 color targets
│   └── preprocess_id_photo.py    # Formats raw selfies into CUFS-style setups
...
This runs Phase 1 and Phase 2 back to back in one command on raw input (assuming the image is already CUFS-style formatted).

### Full Pipeline with Preprocessing Options

If you have a raw selfie photo and want to map it to the CUFS ID-photo domain before generating the sketch and color image, you can enable built-in preprocessing:
...
The pipeline supports extensive optional arguments for data extraction and debugging:
...
python pipeline/generate_full_pipeline.py \
	...
	--output final_portrait.png \
	--enable-preprocess \
	--preprocess-output intermediate_id_photo.png \
	--preprocess-skin-strength 0.82 \
	--preprocess-shadow-fix-strength 0.70 \
	--phase1-sketch-output intermediate_sketch.png
```

- `--enable-preprocess`: Triggers background removal, blue-background compositing, lighting normalization, and face adjustments.
- `--preprocess-output`: Saves the formatted ID-style image before it goes into Phase 1.
- `--preprocess-skin-strength`: (0.0 to 1.0) Modifies the skin fairness intensity. Default is 0.75. Try 0.80-0.90 for a stronger fairer tone.
- `--preprocess-shadow-fix-strength`: (0.0 to 1.0) Reduces dark patches around the cheeks and mouth. Default is 0.50. Try 0.60-0.75 to combat harsh local shadows that sketch-generators tend to amplify.
- `--phase1-sketch-output`: Saves the intermediate pencil sketch generated by Phase 1 before it goes into the colorizer in Phase 2.

## Standalone Selfie Preprocessing

If you just want to run the ID-photo formatter without running the deep learning pipeline, use the standalone script:

```bash
python scripts/preprocess_id_photo.py \
	--input path/to/selfie.jpg \
	--output formatted_selfie.png \
	--skin-strength 0.85 \
	--shadow-fix-strength 0.65
```

This effectively uses `rembg`, OpenCV, and custom math to separate the subject, fix skin lighting, and composite cleanly onto a standard blue background.
...
```bash
python pipeline/generate_full_pipeline.py --input path/to/selfie.jpg --output portrait.png --enable-preprocess
```
